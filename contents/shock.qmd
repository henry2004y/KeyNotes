...
In other words, it is convenient to transform to a frame which moves at the local $\mathbf{E}\times\mathbf{B}$ velocity of the plasma. The key idea is to extract the velocity component perpendicular to $\mathbf{B}_1$ from $\mathbf{v}_1$. One possibility is to just remove the perpendicular part:
$$
\begin{aligned}
...
$$
\begin{aligned}
\mathbf{v}_{\mathrm{dHT}} = \mathbf{v}_1 - \frac{V_x}{B_x}\mathbf{B} \\
\mathbf{v}_1^\prime = \mathbf{v}_1 - \mathbf{v}_\mathrm{dHT} = \frac{V_x}{B_x}\mathbf{B}
\end{aligned}
$$
