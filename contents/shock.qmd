...
The different scales can be organised with respect to the different regions of the shock.

1. The macroscopic scale of the foot region, which determines the width of the foot, is the ion gyroradius based on the inflow velocity $r_\text{ci,1} = V_1/\omega_\text{ci,1}$. With the slight modification of replacing the upstream magnetic field with the (inhomogeneous) ramp magnetic field $B_r(x)$ this also becomes approximately the scale of the macroscopic electric potential drop in the ramp, $\Delta_{\phi, r}\sim r_\text{ci, r}\sim V_1 / \omega_\text{ci,r}$.
...
3. The thermal ion gyroradius $r_{ci} = v_\text{th,i}/\omega_\text{ci}$. It determines the transition from unmagnetized to magnetized ions and from non-adiabatic to adiabatic heating of the ions.
