...
The de Hoffmann-Teller velocity is the same to both sides of the shock ramp, because of the continuity of normal component $B_n$ and tangential electric field $\mathbf{E}_t$. Thus, in the de Hoffmann-Teller frame there is no induction electric field $\mathbf{E} = −\hat{n}\times\mathbf{U}\times\mathbf{B}$. The remaining problem is two-dimensional (because trivially $\hatn$, $\hat{b}$ and $−v_\parallel\hat{b}$ are coplanar, which is nothing else but the coplanarity theorem holding under these undisturbed idealized conditions).
...
The unit vectors $\hat{x}$, $\hat{y}$ are along the orthogonal coordinates in the gyration plane of the ion, the phase $\hi_0$ accounts for the initial gyro-phase of the ion, and $\pm$ accounts for the direction of the upstream magnetic field being parallel (+) or antiparallel (-) to $\hat{b}$.
