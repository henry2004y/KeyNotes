...
### Stability of magnetic mirror in the scope of single particle motion
...
To understand why shear flow can lead to instability, we'll first introduce the Bernoulli theorem in fluid mechanics. From the MHD force law, take $\frac{\partial}{\partial t}=0,\ \mathbf{J}=0,\ \mathbf{g}=0$, we obtain
...
Using the natural coordinates, let $\hat{t}$ be the unit tangent vector on a streamline, $\hat{n}$ be the unit normal vector pointing from concave to convex side, and $ds$ be the incremental distance along streamline, we have

$$
\begin{aligned}
\rho\mathbf{v}\cdot\nabla\mathbf{v} &= (\rho v\frac{\partial }{\partial s})\mathbf{v} =\rho v_s \frac{\partial }{\partial s}[\hat{t} v] \\
&=\rho v \big[ \frac{\partial v}{\partial s}\hat{t} + \frac{\partial \hat{t}}{\partial s}v\big] \\
...
Therefore, $\frac{1}{2}\rho v^2 + p = \text{const.}$ along a streamline for an incompressible flow.

Now, consider two flow layer with velocity shear at the plane interface in Fig.\ref{fig:two_layer}. Imaging there`s a ripple on the layer interface pointing upward. If we examine the cross section at $P$ and $Q$ for the lower layer respectively, we find
...
Since density along field lines are constant, $A_P<A_Q$, we have

$$
v_P>v_Q \Rightarrow P_P<P_Q
$$

Similar for the upper layer, we get the pressure at P is larger than that at Q. Therefore, the total pressure is pointing away from interface, which let the ripple grow.

![Two layers of flow with a velocity shear at the plane interface.](images/electron_sheet.png){#fig:two_layer_flow}

Now we are ready to do derivations. The governing equations are
...
In equilibrium, suppose there is velocity shear in x direction, and the interface is in y direction,
...
For perturbations, assume

$$
\begin{aligned}
v_{1x}(\mathbf{x},t) &= v_{1x}(x)e^{-i\omega t+i k_y y} \\
p_1(\mathbf{x},t) &= p_1(x)e^{-i\omega t+i k_y y}
\end{aligned}
$$

so the linear equation of momentum is
...
Now we are half way through the dispersion relation. For simplicity, set the velocity on the two sides $v_1 = 0,\ v_2 = v_1+\Delta v \equiv v$, and $k=k_y$. So

$$
v_{0y}^{\prime\prime}(x) = \Big(\frac{v}{\tau}\Big)\big[ \delta(x) - \delta(x-\tau)\big]
$$

except at $x=0$ and $x=\tau$, Eq.(\ref{eq:KH_governing}) can be simplified to

$$
\frac{d^2 v_{1x}}{dx^2}-k^2 v_{1x} = 0
$$

Let the shear layer thickness be $\tau$.
...
In region II ($x\in(o,\tau)$)

$$
\begin{aligned}
v_{1x} &=\xi_\tau \frac{\sinh{kx}}{\sinh{k\tau}} + \xi_0\frac{\sinh{k(x-\tau)}}{\sinh{-k\tau}},\quad x>\tau \\
...
$$
\begin{aligned}
v_{1x} &= \xi_\tau e^{kx},\quad x<0 \\
\frac{\partial v_{1x}}{\partial x}\big|_{x=0_-} &= k\xi_0
\end{aligned}
$$

Integrate the governing @eq:KH_governing} from $x=0^-$ to $x=0^+$, yielding

$$
-\xi_0 k\frac{\cosh{k\tau}}{\sinh{k\tau}} + \xi_\tau k\frac{1}{\sinh{k\tau}} - k\xi_0 + \frac{kv}{\omega\tau}\xi_0 = 0
$$

Integrate the governing @eq:KH_governing from $x=\tau^-$ to $x=\tau^+$, yielding
...
$$
\sinh{k\tau} + \cosh{k\tau} = e^{k\tau}
$$

the dispersion relation can be simplified to
...
(TO BE COMPLETED AND CORRECTLY CATEGORISED!)
...
For MELBA-like beam with the following parameters, $V=700keV,\ I=1kA,\ a = 5cm,\ \tau=0.5cm,\ T=500ns,\ L=1m,\ B=2kG$,

$$
\omega_i =1.18\times 10^7 s^{-1}
$$
