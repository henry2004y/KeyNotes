...
There are five basic quantities in the hybrid model (length, mass, time, current density, and number density???) and three physical constants ($\mu_0,q,m$). If we add temperature, then correspondingly $k_B$ would appear. Thus we need two independent reference quantities for the normalized units in a hybrid model. For instance, We can take a magnetic field scale $B_0$, number density scale $n_0$. The general variable transformation from the original units to normalized units is
$$
\chi = \chi_0\tilde{\chi}
$$
...
$$
P_0 = \frac{B_0^2}{2\mu_0} = \frac{1}{2}\rho_0 v_0^2
$$
...
| Number density | $n_0$    | $n_0=n_\mathrm{ref}$ | $1\times10^{7}\,\mathrm{m}^{-3}$ |

: Example basic quantities in the hybrid model

| Derived variable | Notation      | Definition | Value |
|------------------|---------------|------------|-------|
| Length           | $l_0$         | $l_0=\sqrt{m_0/(\mu_0 e^2 n_0)}$ | $7.2\times10^4\,\mathrm{m}$ |
| Mass             | $m_0$         | $m_0=m_i$| $1.67\times10^{-27}\,\mathrm{kg}$ |
| Velocity         | $v_0$         | $v_0=B_0/\sqrt{\mu_0m_in_0}$| $6.90\times10^4\,\mathrm{m}\cdot\mathrm{s}^{-1}$|
| Time             | $t_0$         | $t_0=m_i/(e*B_0)$ | 1.04 s |
| Mass density     | $\rho_0$      | $\rho_0=n_0/m_i$ | $1.67\times10^{-20}\,\mathrm{m}^{-3}$ |
| Pressure         | $p_0$         | $p_0=0.5\rho_0v_0^2$ | $3.98\times10^{-11}\,\mathrm{N}\cdot\mathrm{m}^{-2}$ |
| Temperature      | $T_0$         | $T_0=p_0*m_i/(k_B\,\rho_0)$ | $2.88\times10^5\,\mathrm{K}$ |
