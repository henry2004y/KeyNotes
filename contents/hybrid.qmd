...
We need three independent reference scales for normalized ion units in a hybrid model. For instance, We can take a magnetic field scale $B_0$, number density scale $n_0$, and mass scale $m_0$. The general variable transformation from the original units to normalized units is
