...
1. The _dimensionless_ reconnection rate, defined as the ratio between the inflow magnetic flux and the outflow magnetic flux:
$$
...
The approximation is usually valid when we take the field values far away from the reconnection site. If there is no diffusion, all the incoming magnetic field lines can be reconnected, thus $R=1$. Due to the presence of ion and electron diffusion regions, the magnetic flux will dissipate near the X-line (smaller field strength, smaller velocity), therefore not all of the field lines will be reconnected. Observationally, this ratio is on the order of $\mathcal{O}(0.1)$. There has been some arguments based on geometry to explain this universal ratio in recent years.

2. The _dimensional_ reconnection rate, defined as the difference between the inflow magnetic flux and the outflow magnetic flux:
$$
R \equiv B_\mathrm{in}u_\mathrm{in} - B_\mathrm{out}u_\mathrm{out}
$$ {#eq-reconnection-rate-dimensional}

When we think of a 2D X-line configuration in the X-Z plane, this is equivalent to the out-of-plane electric field $E_y$ in the unit of [V/m]. For global magnetosphere studies, a useful concept called _reconnection efficiency_ $\epsilon$ is defined as ratio of the imposed potential drop to the full possible potential drop across the width of the magnetosphere. The potential drop is calculated from integrating $R$ (or equivalently the out-of-plane electric field) along the X-line. As an example, Ganymede has been observed to possess an unusually high reconnection efficiency about 60% [@zhou2020reconnection]. High $\epsilon$ means either less diffusion or more plasmas diverting around the magnetopause. 
