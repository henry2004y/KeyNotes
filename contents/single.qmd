...
It is well known in classical mechanics that whenever a system has a periodic motion, the action integral $\oint p dq$ taken over a period is a constant of the motion. Here $p$ and $q$ are the generalized momentum and coordinate which repeat themselves in the motion. If a slow change is made in the system, so that the motion is not quite periodic, the constant of the motion does not change and is then called an *adiabatic invariant*.[^adiabatic] By slow here we mean slow compared with the period of motion, so that the integral $\oint p dq$ is well defined even though it is strictly no longer an integral over a closed path. Adiabatic invariants play an important role in plasma physics; they allow us to obtain simple answers in many instances involving complicated motions. There are three adiabatic invariants, each corresponding to a different type of periodic motion.

[^adiabatic]: This invariant has the name adiabatic associated with it because historically it was firstly studied systematically by Paul Ehrenfest in the inversible adiabatic process in thermal dynamics. Later the word "inversible" is neglected and it was called "adiabatic invariant", although this conclusion is valid in a general system that changes slowly that may have nothing to do with thermal dynamics. This also has deep root with quantum mechanics. 在中文中，更加合理的翻译是“浸渐不变量”，而非“绝热不变量”。

A review of adiabatic charged-particle motion is given by @northrop1963adiabatic.
...
The first invariant is derived from the action integral of the fast gyromotion. By expanding the magnetic vector potential $\mathbf{A}$ about the guiding center and integrating the canonical momentum perpendicular to the field over one gyro-period, we obtain the first adiabatic invariant:
$$
I_1 = \oint \mathbf{P}_\perp \cdot d\boldsymbol{\rho} \approx \frac{c}{e} \frac{p_\perp^2}{2B} = \frac{cm}{e} \mu
$$

This yields the magnetic moment $\mu$, which is conserved in the guiding-center limit:
$$
\mu = \frac{p_\perp^2}{2mB}
$$

In the relativistic case, the magnetic moment is related to the adiabatic invariant by $M = \mu/\gamma$.
...
Once the fast gyromotion is averaged out, the remaining motion consists of bouncing and drifting.

Second Invariant ($I_2$ or $J$): Derived by integrating the parallel momentum along the field line between bounce points $s_m$ and $s'_m$
$$
J = \oint p_\parallel ds = 2p \int_{s_m}^{s'_m} \sqrt{1 - \frac{B(s)}{B_m}} ds
$$
Conservation of $J$ implies that as a particle drifts across field lines, it adjusts its bounce path to keep this integral constant.
...
All particle accelerations essentially come from the work of electric field.
Classical theory considers adiabatic mechanisms; non-adiabatic mechanisms like interactions with turbulence or wave-particle interactions are added later. One criterion for checking the validity of adiabatic assumption is given as
$$
\kappa = \sqrt{R_C / R_L} > \kappa_\mathrm{set}
$$
where $R_C$ is the radius of curvature, and $R_L$ is the particle gyro-radius. If $\kappa$ is large, it means that the characteristic length of the changing magnetic field structure is large compared to the gyromotion. In practice $\kappa_\mathrm{set} \simeq 3$ is usually used.

There are three basic mechanisms of adiabatic acceleration:

* _Betatron acceleration_ refers to situations in which the magnetic field strength increases slowly in time (compared with a gyroperiod, i.e. adiabatic assumption), so that $\mu$ remains constant, but the particle kinetic energy is not constant due to the presence of electric fields (associated with the time-varying magnetic field). Then, the perpendicular energy is increased due to constancy of $\mu$. One simple model to test betatron acceleration is to determine the change in energy based on the change in magnetic field.

* _Fermi acceleration_, is the acceleration that charged particles undergo when streaming along and drifting in relaxing curved magnetic field lines. It takes place as particles are repeatedly reflected, usually by a magnetic mirror.[^mirror] Fermi acceleration is a result of the conservation of the second adiabatic invariant and accelerates particles as the length of flux tubes decrease. Another form of Fermi acceleration seen in the tail comes in “reflections”, which is analogous to the classical system of a ball gaining energy as it bounces off two walls moving towards each other. There are two types of Fermi acceleration: _first-order Fermi acceleration_ (in shocks) and _second-order Fermi acceleration_ (in the environment of moving magnetized gas clouds) [@axford1977acceleration]. The first-order Fermi acceleration involves particles gaining energy through head-on collisions with moving magnetic structures, like shock waves. The energy gain per collision is proportional to the shock velocity. The second-order Fermi acceleration involves particles gaining energy through random scattering off of moving magnetized clouds. The energy gain per collision is proportional to the square of the cloud velocity, making it less efficient than first-order acceleration. In both cases the environment has to be collisionless with pre-existing medium energy particles in order for the mechanism to be effective. This is because Fermi acceleration only applies to particles with energies exceeding the thermal energies, and frequent collisions with surrounding particles will cause severe energy loss (heating) and as a result no acceleration will occur. See more in [wiki](https://en.wikipedia.org/wiki/Fermi_acceleration) and [@fermi1949origin].

* _localized electric fields_ $E_\parallel$ (parallel to the magnetic field), which directly accelerate particles. It is found that direct acceleration by parallel electric fields in structures like dipolariztion fronts is smaller than observation error.

[^mirror]: Mirror structures trap electrons, which gives further time and space for the Fermi acceleration process. There are observation evidence that high electron acceleration can be associated with the favorable occurrence of mirror modes. Likely, the merging of flux ropes is also favorable for Fermi acceleration and direct reconnected electric field acceleration near the diffusion region.

It is important to point out the difference between acceleration and heating. When we consider the motion of single particles, there is no such concept as temperature, thus no heating exists.
However, acceleration of a bunch of particles can lead to an extension of the velocity distributions, therefore increasing the temperature.
In practice, people use the term "acceleration" in the cases of high-energy tails, and "heating" in the cases of the thermal populations.

In the shock scenario, we have the following acceleration types:

* _diffusive shock acceleration_ (DSA). DSA is actually considered a subclass of Fermi acceleration, specifically first-order Fermi acceleration. The "diffusive" in DSA refers to the scattering of particles by magnetic turbulence, which allows them to repeatedly cross the shock front.

* _Shock drift acceleration_ (SDA). Gradients within the shock transition region may give rise to the phenomenon of Shock Drift Acceleration (SDA), where ions travel along the shock front and gyrate through the ramp multiple times before either passing into the downstream or escaping into the upstream region. This drift is perpendicular to both the magnetic field and the shock normal. The particle gains energy from the electric field as it drifts along the shock front, where the electric field can be generated from a sudden compression of the plasma.

* _Shock surfing acceleration_ (SSA). SSA occurs when a charged particle is "trapped" in the electric potential of the shock front. The particle is reflected back and forth across the shock front by the magnetic field, effectively "surfing" on the shock wave. The particle gains energy from the electric field during each reflection.

* _Stochastic shock drift acceleration_ (SSDA). SSDA adds to the general SDA theory by introducing a stochastic process where the accelerated electrons are pitch-angle scattered by whistler waves in the shock transition layer, allowing electrons to undergo the SDA process longer and hence gain more energy.

Both SDA and SSA are important mechanisms for pre-accelerating particles, meaning they can inject particles into further acceleration processes like DSA. DSA is the dominant mechanism for producing high-energy cosmic rays. 
Pitch angle distributions (PADs) are an important tool to distinguish between betatron and Fermi types of acceleration.
Since betatron acceleration acts on perpendicular particles, it produces PADs peaked around $90^\circ$, sometimes called a "pancake distribution".
On the other hand, Fermi acceleration acts on parallel particles, so it produces a field-aligned and anti-field-aligned distribution, known as a "cigar distribution". 

Under adiabatic assumptions and guiding center approximations, the evolution of a particle energy $\mathcal{E}$ can be written as (@northrop1963adiabatic, @dahlin2014mechanisms)
$$
...
where $\mu=m \gamma^2 v_\perp^2/(2B)$ is the magnetic moment, $\gamma$ is the relativistic Lorentz factor, and $\mathbf{v}_c$ and $\mathbf{v}_g$ are the curvature and gradient drift velocities
$$
...
$\Omega_{c} = eB/(\gamma m c)$ is the cyclotron frequency, and $\pmb{\kappa} = \mathbf{b}\cdot\nabla\mathbf{b}$ is the curvature. @eq-particle_energization_nonrelativistic1 can equivalently be written as
$$
...
@eq-particle_energization_nonrelativistic2 describes the acceleration process in terms of ExB drift, where particles accelerate when drifting towards stronger B field or B curvature direction.
The first term is the acceleration by the parallel electric field. The second term corresponds to perpendicular heating or cooling due to the conservation of $\mu$.
The third term drives parallel acceleration and arises from the first-order Fermi mechanism.
For example, newly reconnected field lines downstream from a reconnecting X-line accelerate as a result of the tension force ($\sim B^2\pmb{\kappa}$) and "straighten".
Particles that reflect from this moving field line receive a Fermi "kick" and thereby gain energy.
A particle trapped in an elongated magnetic island whose ends are contracting due to these tension forces will repeatedly gain energy as it reflects from the ends of the island.
More generally, any particle reflecting from this moving bent field line will gain energy.

If we integrate all the particles in the phase space from @eq-particle_energization_nonrelativistic2, the total particle energy change can be written as (non-relativistic case)
$$
...
The terms on the RHS are the parallel electric field acceleration, betatron acceleration, and Fermi acceleration, respectively. Depending on the drift terms on the RHS of @eq-particle_energization_nonrelativistic1, additional terms can be added, such as the polarization drift as shown below.

Particle accelerations are important consequences of magnetic reconnection and shocks.
In magnetic reconnection processes, the energy is rapidly converted from magnetic field to particles in the forms of flows, bulk ("thermal") heating, and high-energy ("nonthermal") tails.
[@ji2022magnetic] describes the current understanding of electron acceleration during magnetic reconnection in the non-relativistic and relativistic regimes.
First-principles numerical simulations suggest power-law nonthermal particle populations are more prevalent in systems with low $\beta$ and moderate guide (perpendicular to the reconnection plane) magnetic fields, with acceleration efficiency enhanced in 3D compared to simplified 2D calculations.
Multiple acceleration mechanisms have been studied.
In the localized diffusion region near an X-line without a guide field, particles may become demagnetized, prone to be directly accelerated by the reconnection electric field as they undergo so-called _Speiser_ or _meandering orbits_.
There is recent laboratory evidence for electron acceleration by this mechanism in action during magnetically driven reconnection at low-$\beta$ using laser-powered capacitor coils.
There is also numerical evidence that a substantial fraction of dissipated magnetic energy is carried by particles accelerated by this mechanism when they are demagnetized and escape from plasmoids in 3D.

Over larger scales when the particles remain magnetized, the three basic particle acceleration processes dominate in collisionless reconnection.
The energization rate in the non-relativistic limit is given approximately by the following equation:
$$
...
where $\mathcal{E}$ is the energy of a particle, $\mathbf{u}_E$ is the $\mathbf{E}\times\mathbf{B}$ drift velocity, $\frac{\mathrm{d}}{\mathrm{d}t}=\frac{\partial}{\partial t} + \mathbf{u}_E\cdot\nabla$, $E_\parallel$ is the parallel electric field, and $v_\parallel$ is the drift-corrected guiding center parallel velocity.
For slowly varying fields, $\mathbf{u}_c$ is the curvature drift of particles and reduces to $\mathbf{u}_c \sim(mv_\parallel^2/qB)(\hat{b}\times\pmb{\kappa})$, where $\pmb{\kappa}=\hat{b}\cdot\nabla\hat{b}$ is the magnetic field curvature and $\hat{b}$ is the unit vector in the direction of $\mathbf{B}$.
The terms on the right-hand side represent energy gain by the parallel electric field, betatron heating, Fermi acceleration, and the polarization drift respectively.
The first three mechanisms are sketched in @fig-particle-acceleration-reconnection. The polarization drift is particularly important for lower-energy ions, and this terms accounts for the kinetic energy gain in the large-scale reconnection outflow jets.
Although @eq-particle_energization_nonrelativistic3 is valid in the non-relativistic limit, the same basic acceleration mechanisms are responsible for energizing particles in more extreme relativistic contexts relevant to astrophysical systems.

::: {.content-visible unless-format="pdf"}

![Illustration of particle acceleration mechanisms. The reconnection electric field is along the $y$ direction.
Acceleration by the reconnection electric field occurs near the X-line, which is associated with the Speiser orbit, and by parallel electric field near separatrices, which are boundaries between upstream (before-reconnection) and downstream (after-reconnection) regions. When particles near the X-line interact with the parallel electric field, ions (electrons) are accelerated along the (opposite) direction of the field, and the direct acceleration term $qE_\parallel v_\parallel$ (the first term on the right side of @eq-particle_energization_nonrelativistic3) is positive. The betatron acceleration mechanism happens when particles flow into the stronger magnetic field regions in the exhaust. In this example trajectory, particles move along the positive z direction by the $\mathbf{E}\times\mathbf{B}$ motion and $\nabla B$ is also along the positive z direction. As a result, the second term of @eq-particle_energization_nonrelativistic3 is positive. Fermi acceleration occurs in the regions with high-curvature magnetic field lines. In this example, plasmoids are elongated along the $z$ direction, such that the field lines have the highest curvature at $x = 0$. Inside the plasmoid, particles obtain a large velocity kick along the $y$ direction due to a large curvature drift and obtain a significant energy from the reconnection electric field ($q\mathbf{E}\cdot\mathbf{u}_c > 0$). In the exhaust region, trapped particles gain energy whenever they pass the high curvature regions at $x = 0$. di is ion skin depth. Figure 5 from [@ji2022magnetic]](https://media.springernature.com/lw685/springer-static/image/art%3A10.1038%2Fs42254-021-00419-x/MediaObjects/42254_2021_419_Fig5_HTML.png){#fig-particle-acceleration-reconnection}
