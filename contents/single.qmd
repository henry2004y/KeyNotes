...
Classical theory considers adiabatic mechanisms; non-adiabatic mechanisms like interactions with turbulence or wave-particle interactions are added later.
...
@eq-particle_energization_nonrelativistic2 describes the acceleration process in terms of ExB drift, where particles accelerate when drifting towards stronger B field or B curvature direction. The first term is the acceleration by the parallel electric field. The second term corresponds to perpendicular heating or cooling due to the conservation of $\mu$. The third term drives parallel acceleration and arises from the first-order Fermi mechanism. For example, newly reconnected field lines downstream from a reconnecting X-line accelerate as a result of the tension force ($\sim B^2\pmb{\kappa}$) and "straighten". Particles that reflect from this moving field line receive a Fermi "kick" and thereby gain energy. A particle trapped in an elongated magnetic island whose ends are contracting due to these tension forces will repeatedly gain energy as it reflects from the ends of the island. More generally, any particle reflecting from this moving bent field line will gain energy.
