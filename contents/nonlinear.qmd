...
An intuitive way to understand the parametric decay instability is that one Alfvén mode with wave vector $\mathbf{k}_1$ will decay into a smaller $\mathbf{k}_2$ and a sound wave $\mathbf{k}_s$. The sound wave will trigger density perturbations.
