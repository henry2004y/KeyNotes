...
$\nabla\cdot\mathbf{E}$ is small when $k^2\lambda_{De}\ll 1$, and this simply gives the quasi-neutrality condition.
...
Let $A = (p_\perp - p_\parallel)/p_\parallel$. The system will be unstable if $A<-2/\beta_\parallel$, i.e.
$$
p_\perp - p_\parallel > 2 p_B
$$ {#eq-kmhd_firehose_criterion}
which leads to a growth rate
$$
\gamma = k_\parallel v_{th\parallel}\sqrt{\bigg\lvert A+\frac{2}{\beta_\parallel}\bigg\rvert}
$$ {#eq-kmhd_firehose_growth_rate}

Thus, negative $A$ ($p_\parallel > p_\perp$) locally weakens tention, i.e. slows down Alfvén waves, and makes it energetically easier to bend the field lines. When $A<-2/\beta_\parallel$, the elasticity of field lines is lost and we have the firehose instability.
...
@fig-instability-mirror-firehose shows observation from Wind spacecraft. The solar wind indeed seems to stay within these boundaries. (ADD REFS!)

![Collective solar wind observation data ($\sim 1e6$) from Wind spacecraft. The lines represent the instability thresholds for mirror and firehose instability, respectively.](../images/instability_mirror_firehose_solarwind.png){#fig-instability-mirror-firehose}
