...
$$
\mathrm{d}N = 4\pi p^2 f(\mathbf{r}, p, t)\mathrm{d}^3 r\mathrm{d}p = U_p(\mathbf{r}, p, t)\mathrm{d}^3 r\mathrm{d}p
$$
where we define the differential number density $U_p$ as the number of particles in $\mathrm{d}^3 r$ about $\mathbf{r}$ with momentum in the interval $(p, p+\mathrm{d}p)$ as
$$
U_p(\mathbf{r}, p, t) = 4\pi p^2 f(\mathbf{r}, p, t)
$$
with units of [particles per volume].

Detectors do not measure $U_p$, but rather differential intensity. If particle velocity has magnitude $v$, the differential intensity is
$$
j_p = v\, U_p
$$
...
$$
j_T = \frac{U_p}{4\pi} = p^2 f(\mathbf{r}, p, t)
$$
That is, the differential intensity measured by experimentalists has simple relationship to quantities $U_p$ and $f$ used by theorteticians.
...
$$
or by using $U_p=4\pi p^2 f$ we have
$$
\frac{\mathrm{d}U_p}{\mathrm{d}t} = \frac{\partial U_p}{\partial t} + \nabla\cdot\mathbf{S}_p + \frac{\partial}{\partial p}\left( \left< \dot{p} \right> U_p \right)
$$
where $\mathbf{S}_p = \left< \mathbf{v} \right> U_p$ is the differential flux. Normally $\frac{\mathrm{d}U_p}{\mathrm{d}t} = 0$ and the above is simply a continuity equation. If there exist a source or sink $Q_p$, one can write
$$
\frac{\partial U_p}{\partial t} + \nabla\cdot\mathbf{S}_p + \frac{\partial}{\partial p}\left( \left< \dot{p} \right> U_p \right) = Q_p
$$ {#eq-cr-transport}
...
- Solar Wind Frame: We often work in a frame moving with the solar wind (or background plasma) velocity $V_\text{sw}$.
- Negligible Large-Scale Electric Field in Solar Wind Frame: In the frame co-moving with a perfectly conducting solar wind, the large-scale electric field is typically assumed to be zero, i.e., $\mathbf{E}^\prime = \mathbf{E} + \mathbf{V}_\text{sw} \times \mathbf{B} \approx 0$. This means $\mathbf{E}\approx -\mathbf{V}_\text{sw} \times \mathbf{B}$.
- Omnidirectional Distribution Function: We are often interested in the omnidirectional distribution function $f_0(x,p,t)$, which is $f$ averaged over all momentum directions in the plasma frame. Or, more commonly, the differential number density $U_p( 
x,p,t)=4\pi p^2\,f_0​(x,p,t)$ (number of particles per unit volume per unit momentum magnitude) or $N(x,T,t)$ (differential intensity w.r.t. kinetic energy $T$).
...
$$
f(\mathbf{x}, p, t) = f_0(\mathbf{x}, p, t) + f_1(\mathbf{x}, p, t) + ...
$$
where $f_0$ is the isotropic part and $f_1$ is the first-order anisotropy.
$$
f_1 = -\frac{\mathbf{p}}{p}\cdot \pmb{\xi}(\mathbf{x}, p, t)
$$
where $\pmb{\xi}$ is the anisotropy vector. The particle flux (or streaming) $\mathbf{S}_\text{cr}$ is related to this anisotropy.

The collision term describes the scattering of cosmic rays by magnetic field irregularities. In the diffusion approximation, this scattering tends to isotropize the distribution in the plasma rest frame. This process leads to spatial diffusion. The diffusive flux $S_\text{diff}$ in the plasma frame is given by Fick's law:
$$
\mathbf{S}_\text{diff} = - \overleftrightarrow{K}\cdot\nabla U_p
$$
where $U_p$ is the cosmic ray density (related to $f_0$) and $\overleftrightarrow{K}$ is the diffusion tensor. The diffusion tensor is generally anisotropic:
$$
K_{ij} = K_\parallel b_i b_j + K_\perp(\delta_{ij} - b_i b_j) + K_A \epsilon_{ijk} b_k
$$

- $K_\parallel$ is the diffusion coefficient parallel to the mean magnetic field direction $\mathbf{b}$.
- $K_\perp$ is the diffusion perpendicular to $\mathbf{b}$.
- $K_A$ is the antisymmetric part of the diffusion tensor describing gradient and curvature drifts.

Starting from the kinetic equation and averaging over pitch angle (assuming near isotropy in the plasma frame), one can derive an equation for $f_0$. This is a lengthy process (e.g., see Schlickeiser 2002, "Cosmic Ray Astrophysics" or Parker 1965). The key steps involve:

- Transforming the kinetic equation to the plasma frame.
- Expanding $f$ in momentum space in the plasma frame ($f=f_0 + f_1 + ...$).
- The zeroth moment (integration over solid angle in momentum) of the kinetic equation gives the transport equation.
- The first moment gives a relation for the anisotropic part $f_1$ (streaming).
...
\end{aligned}
$$
where $f(\mathbf{x}, p, t)$ is the particle distribution function as a function of the particle position $\mathbf{x}$, momentum $p$ (isotropic momentum assumed), and time $t$; $\overleftrightarrow{K}$ is the spatial diffusion coefficient tensor, $\mathbf{V}$ is the bulk plasma velocity, $\mathbf{V}_d = \frac{pv}{3q}\nabla\times\left( \mathbf{B}/B^2 \right)$ is the particle drift, where $p=\gamma m v$ is particle momentum and $q$ is particle charge.[^form] The factor $1/3$ comes from averaging over an isotropic pitch-angle distribution. Note that This is for an isotropic distribution; this term is sometimes incorporated into the anti-symmetric part ($K_A$) of the diffusion tensor $\overleftrightarrow{K}$. If so, the diffusion term becomes $\nabla\cdot\left( \overleftrightarrow{K}_s\cdot\nabla f_0 - \left< v_D \right> f_0 \right)$ where $\overleftrightarrow{K}_s$ is the symmetric part.

- $\frac{1}{3}\nabla\cdot\mathbf{V}\frac{\partial f}{\partial \ln{p}}$ is the adiabatic energy change. The sign of $\nabla\cdot\mathbf{V}$ reflects whether cosmic rays lose or gain energy from the compression of the solar wind plasma.

[^form]: This is the form given in the [GPAT model](https://stochastic-parker.readthedocs.io/en/latest/theory/parker_intro.html). Riaan Steenkamp shows a slightly different version for the guiding center drift in his notes which absorbs the adiabatic energy change into the diffusion tensor. Also note that $\frac{\partial f}{\partial \ln{p}} = p\partial f_0 / \partial p$.
