...
j_p = \frac{v\, U_p}{4\pi}
$$
with units of [particles / unit momentum / unit area / unit time / unit radian]. In practice we measure particles w.r.t. kinetic energy intervals $\mathrm{d}T$ but not momentum intervals $\mathrm{d}p$. From $E^2 = p^2 c^2 + E_0^2$ we have $\mathrm{d}T = v\mathrm{d}p = \mathrm{d}(mv^2/2)$. Since we specify the same particles in two ways, we have $j_T\mathrm{d}T = j_p\mathrm{d}p$ and
$$
...
Given $f = U_p / (4\pi p^2)$, we can write ... as
$$
\begin{aligned}
\frac{\partial f}{\partial t} + (\mathbf{V} + \mathbf{V}_d)\cdot\nabla f - \frac{1}{3}\nabla\cdot\mathbf{V}\frac{\partial f}{\partial \ln{p}} = \nabla\cdot \left( \pmb{\kappa} \nabla f \right) + Q
\end{aligned}
$$
where $f(\mathbf{x}, p, t)$ is the particle distribution function as a function of the particle position $\mathbf{x}$, momentum $p$ (isotropic momentum assumed), and time $t$; $\pmb{\kappa}$ is the spatial diffusion coefficient tensor, $\mathbf{V}$ is the bulk plasma velocity, $\mathbf{V}_d = \frac{pv}{3q}\nabla\times\left( \mathbf{B}/B^2 \right)$ is the particle drift, where $p=\gamma m v$ is particle momentum and $q$ is particle charge.[^form]

[^form]: This is the form given in the [GPAT model](https://stochastic-parker.readthedocs.io/en/latest/theory/parker_intro.html). Riaan Steenkamp shows a slightly different version for the guiding center drift in his notes.
