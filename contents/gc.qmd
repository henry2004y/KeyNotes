...
- Term 1 ($ \propto \mathbf{B}^*$): Represents motion along the field line plus curvature drift (hidden inside $\mathbf{B}^*$).
- Term 2 ($ \propto \mathbf{E}^ \times \mathbf{b}$):* Represents the $\mathbf{E} \times \mathbf{B}$ drift and the $\nabla B$ drift (hidden inside $\mathbf{E}^*$ via the $\mu \nabla B$ term).
- Denominator: $B^*_{\parallel} = \mathbf{b} \cdot \mathbf{B}^* \approx B$. This is a normalization factor essentially equal to $B$ but corrected for phase-space volume conservation.

Equation for Parallel Momentum ($\dot{p}_{\parallel}$):
$$
\frac{dp_{\parallel}}{dt} = \frac{q}{B^*_{\parallel}} \mathbf{B}^* \cdot \mathbf{E}^*
$$
