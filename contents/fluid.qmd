...
1. Ionized $\textrm{H}^+$ plasma: we use $e,\, i,\, a$ as subscripts for electron, ion and neutral species respectively.
$$
\begin{aligned}
q_e=-q_i=-e \\
...
3. Ideal gas in thermal equilibrium
$$
\begin{aligned}
T_e=T_i=T_a \\
p=p_e+p_i+p_a
...
4. Flow velocity is much smaller than thermal speed,
$$
...
5. Ignore viscosity and non-EM force.
...
\mathbf{u}_a&=\mathbf{u}-\frac{n_i}{n_a}(\mathbf{v}_i+\frac{m_e}{m_i}\mathbf{v}_{ei})
\end{aligned}
$$
...
\mathbf{I}_{ea}&=m_e\Big[\Big( 1+\frac{n_im_e}{n_a m_i}\Big)\mathbf{v}_{ei}+\Big( 1+\frac{n_i}{n_a}\Big)\mathbf{v}_i\Big]\approx m_e\Big[ \mathbf{v}_{ei}+(\frac{1}{1-\alpha})\mathbf{v}_i \Big] \\
...
$$
\begin{aligned}
m_en_e\frac{d\mathbf{u}_e}{dt}&=m_en_i\frac{d}{dt}(\mathbf{v}_{ei}+\mathbf{v}_i+\mathbf{u}) \\
&=-\nabla p_e - n_i e \big[ \mathbf{E}+(\mathbf{v}_{ei}+\mathbf{v}_i+\mathbf{u})\times\mathbf{B} \big]+\mathbf{f}_e^c
\end{aligned}
$$ {#eq-electron_motion_ohm}
where the collisional force
$$
\mathbf{f}_e^c=-n_i m_e\mathbf{v}_{ei}(\tau_{ei}^{-1}+\tau_{ea}^{-1})-\frac{1}{1-\alpha}n_i m_e\mathbf{v}_i\tau_{ea}^{-1}
$$
...
$$
\mathbf{f}_e^c=-n_i\mathbf{I}_{ei}\tau_{ei}^{-1}-n_i\mathbf{I}_{ea}\tau_{ea}^{-1}=-n_i m_e\mathbf{v}_{ei}(\tau_{ei}^{-1}+\tau_{ea}^{-1})-\frac{1}{1-\alpha}n_i m_e\mathbf{v}_i\tau_{ea}^{-1}
$$
...
$$
m_i n_i\frac{d\mathbf{u}_i}{dt}=m_i n_i\frac{d}{dt}(\mathbf{v}_i+\mathbf{u})=-\nabla p_i+n_i e\big[ \mathbf{E}+(\mathbf{v}_i+\mathbf{u}\times\mathbf{B}) \big]+\mathbf{f}_i^c
...
In the rest frame of the moving conductor this term drops out because $\mathbf{v}=0$. There is no contradiction because the electric field in the rest frame differs from the E-field in the lab frame: $\mathbf{E}^\prime = \mathbf{E} + \mathbf{u}\times\mathbf{B}$. Electric and magnetic fields are relative and related by theLorentz transformation.

If the current $\mathbf{J}$ is alternating because the applied voltage or E-field varies in time, then reactance must be added to resistance to account for self-inductance (see electrical impedance). The reactance may be strong if the frequency is high or the conductor is coiled. hyzhou:Ponderamotive force???

### Derivation

Negligible terms:

* $\frac{d\mathbf{v}_e}{dt}$ in @eq-electron_motion_ohm and $\frac{d\mathbf{v}_i}{dt}$ in @eq-proton_motion_ohm), because $|\mathbf{v}|\ll |\mathbf{v}^{th}|$

* $\frac{d\mathbf{u}}{dt}$ in @eq-electron_motion_ohm. From @eq-plasma_motion_ohm and $m_e \ll m_i$,
$$
m_en_i\frac{d\mathbf{u}}{dt}=\frac{m_e}{m_i}\frac{n_i}{n_i+n_a}(-\nabla p-n_i e\mathbf{v}_{ei}\times\mathbf{B})\ll 1
$$
...
& -n_i m_e\mathbf{v}_{ei}(\tau_{ei}^{-1}+\tau_{ea}^{-1})-\frac{1}{1-\alpha}n_i m_e\mathbf{v}_i\tau_{ea}^{-1}
\end{aligned}
$$ {#eq-Ohm_derivation1}

@eq-Ohm_derivation1, @eq-proton_motion_ohm and @eq-plasma_motion_ohm are three main equations we need to derive the generalized Ohm's law.
...
$$
\frac{1}{\alpha}m_in_i\frac{d\mathbf{u}}{dt}&=-\nabla p+\mathbf{j}\times\mathbf{B}
$$ {#eq-Ohm_derivation4}

Subtract @eq-Ohm_derivation3 from @eq-Ohm_derivation4 from @eq-Ohm_derivation2, we get the expression for ion current density
$$
\begin{aligned}
\mathbf{j}_i&=\frac{1-\alpha}{(\kappa_{ea}+\kappa_{ia})B}\Big[ \alpha\nabla p -\nabla(p_e+p_i)+(1-\alpha)\mathbf{j}\times\mathbf{B}+(\kappa_{ea}+\frac{\alpha}{1-\alpha}\frac{m_e}{m_i}\kappa_{ia})B\mathbf{j}\Big] \\
&=\frac{1-\alpha}{(\kappa_{ea}+\kappa_{ia})B}\Big[ \frac{\alpha(\alpha-1)}{\alpha+1}\nabla p+(1-\alpha)\mathbf{j}\times\mathbf{B}+(\kappa_{ea}+\frac{\alpha}{1-\alpha}\frac{m_e}{m_i}\kappa_{ia})B\mathbf{j}\Big]
\end{aligned}
$$
...
+\big[1-2r_e(1-\alpha)-\alpha\frac{m_e}{m_i}r_i\big]\mathbf{j}\times\mathbf{B} \\
&+\big[\kappa_{ei}-(1-r_e)\kappa-\frac{\alpha}{1-\alpha}\frac{m_e}{m_i}r_e\kappa_{ia}\big]B\mathbf{j} \\
&+\frac{(1-\alpha)^2}{(\kappa_{ea}+\kappa_{ia})B}\big[ \frac{\alpha}{\alpha+1}\nabla p\times\mathbf{B}-\mathbf{j}\times\mathbf{B}\times\mathbf{B} \big]
\end{aligned}
$$
where
$$
...
\frac{(1-\alpha)^2}{\kappa_{ia}B}\big[ \frac{\alpha}{\alpha+1}\nabla p\times\mathbf{B}-\mathbf{j}\times\mathbf{B}\times\mathbf{B} \big] \\
\mathbf{j}&=\frac{n_i e}{(\kappa_{ei}+\kappa_{ea})B} (\mathbf{E}+\mathbf{u}\times\mathbf{B})+
\frac{1}{(\kappa_{ei}+\kappa_{ea})B}\big( \nabla p_e-\mathbf{j}\times\mathbf{B}\big)-
\frac{(1-\alpha)^2}{\kappa_{ia}(\kappa_{ei}+\kappa_{ea})B}\big[ \frac{\alpha}{\alpha+1}\nabla p\times\mathbf{B}-\mathbf{j}\times\mathbf{B}\times\mathbf{B} \big] 
\end{aligned}
$$
...
$$
\mathbf{j}=\sigma(\mathbf{E}+\mathbf{u}\times\mathbf{B})+\frac{\sigma}{n_i e}(\nabla p_e -\mathbf{j}\times\mathbf{B})
$$
where
$$
