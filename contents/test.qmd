...
Most test parameters are listed in dimensionless units.

If SI units are required, we need unit conversions. Typically, we need a reference number density, e.g. $n_{ref} = 10\mathrm{amu/cc}$, and magnetic field, e.g. $B_{ref} = 10\mathrm{nT}$. The dimensionless scale $\beta$ is also used to determine the unit of temperature.

The reference ion frequency in rad/s is

$$
\omega_{i,\mathrm{ref}} = \sqrt{\frac{n_\mathrm{ref}q_i^2}{\epsilon_0 m_i}}
$$

and then the reference ion inertial length in m, which is also used as the length scale, is

$$
d_i = \frac{c}{\omega_{i,\mathrm{ref}}}
$$
...
$$
v_\mathrm{ref} = \frac{B_\mathrm{ref}}{\sqrt{\mu_0 m_i n_\mathrm{ref}}}
$$

The temperature scale can be derived from the magnetic pressure and $\beta$

$$
T_\mathrm{ref} = \frac{p_B \beta}{n k_B} = \frac{B_\mathrm{ref}^2}{2\mu_0}\frac{\beta}{n k_B}
$$

Inserting the initially chosen values, we get a full set of conversion factors from the normalized units to SI units: $n_\mathrm{ref}, B_\mathrm{ref}, v_\mathrm{ref}, l_\mathrm{ref}, t_\mathrm{ref}$, and $T_\mathrm{ref}$.
...
with $\gamma = 5/3$. If the components of $U$ are ordered $U=[\rho, \rho u_x, \rho u_y, \rho u_z, E]$, then the right-eigenvector for a left-going wave is $R=[1.0, -1.0, 1.0, 1.0, 1.5]$.
...
If $U = [\rho, \rho*u_x, \rho*u_y, \rho*u_z, E, B_y, B_z]$, then the right eigenvectors for left going waves are as follows:
