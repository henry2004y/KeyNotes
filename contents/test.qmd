...
Langmuir wave is a rapid electrostatic oscillation in plasma driven by Coulomb force. We assume immobile ions. From the fluid perspective, the governing equations are given in @eq-langmuir-linear:
$$
\begin{aligned}
...
Langmuir oscillation can be driven self-consistently if there is a density perturbation in electrons, i.e. local charge imbalance. A classical test is to set the 1D spatial domain between $[0,2\pi]$ with 512 points, and time step $\Delta t = 0.01$. The initial perturbation density is $n_1 = 0.02n_0\cos(x)$, where $n_0$ is the uniform background electron density. Temperature is set to 0. Total simulation time is 100.
