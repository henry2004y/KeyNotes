...
$$
t_\mathrm{ref} = \frac{1}{\Omega_i} = \frac{m}{e B_\mathrm{ref}}
$$
...
$$
u_\mathrm{ref} = \frac{l_\mathrm{ref}}{t_\mathrm{ref}} = \frac{B_\mathrm{ref}}{\sqrt{\mu_0 m n_\mathrm{ref}}}
$$
...
In many normalized units, the factor $1/\sqrt{\mu_0}$ is absorbed into the magnetic field,
$$
B^\prime = \frac{B}{\sqrt{\mu_0}}
$$

If CGS units is used, then this shall be (???)
$$
...
where each conversion factor is summarized in @tbl-unit-conversion.

| Variable    | Conversion factor |
|-------------|-------------------|
|$x$          | $\sqrt{\frac{m}{\mu_0 e^2 n_\mathrm{ref}}}$|
|$t$          | $\frac{m}{e\,B_\mathrm{ref}}$|
|$\rho$       | $n_\mathrm{ref}*m$|
|$B$          | $B_\mathrm{ref}$  |
|$u$          | $\frac{B_\mathrm{ref}}{\sqrt{\mu_0 m n_\mathrm{ref}}}$ |
|$p$          | $\frac{B_\mathrm{ref}^2}{2\mu_0}$ |
|$T$          | $\frac{B_\mathrm{ref}^2}{2\mu_0 k_B n_\mathrm{ref}}$ |
|$\mathcal{E}$| $\frac{B_\mathrm{ref}^2}{2\mu_0}$ |

: Unit conversion factors. {#tbl-unit-conversion}

#### Example: dimensionless to $n,u,T$
...
T_\mathrm{SI} &= \frac{(\gamma-1)m\,B_\mathrm{ref}^2}{2\mu_0 k_B n_\mathrm{ref}}\left[ \frac{\mathcal{E}^\prime - {\rho^\prime}^2 - {B^\prime}^2}{\rho^\prime} \right] \\
B_\mathrm{SI} &= B_\mathrm{ref}B^\prime \\
\end{aligned}
$$

When dealing with both unit conversion and perturbations, a recommended way is to first convert the units and then compute the perturbations.
...
Linearization of @eq-ideal-mhd ends up in a system of equations $\mathbf{W}_t = \mathbf{A}(\mathbf{W})\mathbf{W}_x$, where $\mathbf{W}$ is composed of either the primitive variables or conservative variables in 1D. The eigenvalues and eigenvectors of ideal MHD can be used to set the characteristic wave perturbations. See the details in Appendix A and B from [@stone2008athena].
...
with $\gamma = 5/3$. The conservative variables are
$$
...
\end{aligned}
$$
If the conservative variables are ordered as
$$
...
In some cases, we need to convert from conservative variables to other set of variables, e.g. $(n,\mathbf{u},p,\mathbf{B})$. The number density perturbation is
$$
...
$$
\begin{aligned}
\delta p &= (\gamma-1)\left[ \delta\mathcal{E} - \frac{1}{2}(2\rho_0\, u\,\delta u + \rho_0\delta u^2 + \delta\rho\, u_0^2 + 2\delta\rho\,\delta u\, u_0 + \delta\rho\,\delta u^2) - \frac{1}{2\mu_0}\left( 2B_0\delta B + \delta B^2 \right) \right] \\
\end{aligned}
...
$$
\begin{aligned}
\delta T &= \frac{p_0 + \delta p}{k_B\, (n_0+\delta n)} - \frac{p_0}{k_B\, n_0}
\end{aligned}
