...
where each conversion factor is summarized in @tbl-mhd-unit-conversion. Note that in the definition of number density, the denominator is $m_i$ instead of $m_0$ and $R=2k_B/m_i$ only appears in temperature. If you need to compare with a hybrid model, an easy way to make self-consistent conversion is to make mass $m_0=m_i$ as a substitute for $\rho_0$ and then the reference mass density would be $\rho_0=m_0/l_0^3$.
